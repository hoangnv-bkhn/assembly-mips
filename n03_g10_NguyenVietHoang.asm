...
#	Nguyen Viet Hoang - 20184110
#	Ma Lop: 122029

# Do toc do go ban phim va hien thi ket qua bang 2 den LED 7 doan
# 	Cho 1 doan van ban mau, co dinh san trong ma nguon. Vi du: "bo mon ky thuat may tinh"
#	Nguoi dung se nhap cac ky tu tu ban phim
#	Chuong trinh can dem so ky tu duoc go dung va in ra den LED
#--------------------------------------------------------------------------------------------------------------
...
.data 
	...
	orgPattern: 	.asciiz "bo mon ky thuat may tinh" 	# Pattern original
...
Begin:	li	$k0, KEY_CODE              
	...
	li 	$s7, 10						# Bien dung de tinh toan hien thi
  	li 	$s5, 100					# Luu gia tri so vong lap.
	
	jal INPUT						# Procedure nhan string tu nguoi dung
	nop
	jal HANDLING						# Procedure so sanh string duoc nhap voi string goc
	nop
...
INPUT:
	...
	LOOP: 		
	WAIT_FOR_KEY:	lb 	$t1, 0($k1) 			# $t1 = [$k1] = KEY_READY
 			nop
 			beq 	$t1, $zero, CONTINUE 		# if $t1 == 0 then Polling
 			nop
 	INCRE_CHAR:	addi 	$t6,$t6,1    			# Tang bien dem ky tu nhap duoc trong 1s len 1
			teqi 	$t1, 1                       	# if $t1 = 1 tao ngoai le 
			nop
			beq	$v0, 1, INCOM			# $v1 = 1 -> ket thuc string boi '\n' -> out procedure
			nop
	CONTINUE:						# Check 100 vong(1s) 
			addi    $s3, $s3, 1      		# Dem so vong lap
			div 	$s3, $s5			# Lay so vong lap chia cho 100 de xac dinh da duoc 1s hay chua
...
HANDLING:
	...
	CHECK_STRING:			
			...
			nop					# de so sanh voi ky tu thu $t1 o orgPattern
			move 	$a0, $t5			
			syscall 
			nop
			la 	$t4, orgPattern
			add 	$t4, $t4, $t1			# Lay vi tri ki tu thu $t1
			lb 	$t6, 0($t4)			# Lay ky tu thu $t1 trong orgPattern luu vao $t6
			nop
